...
def test_no_lut(no_lut):
    ...
    phot = MockPhotometer(lut=no_lut)
...
def test_lut_array(nonlinear_lut):
    ...
    phot = MockPhotometer(lut=nonlinear_lut)
...
def test_lut_callable():
    phot = MockPhotometer(lut=lambda x: x * 200.0)
...
def test_noise_mean_close_to_true(linear_lut):
    ...
    phot = MockPhotometer(lut=linear_lut, noise=0.5, rng=42)
    phot.current_intensity = 0.5
