...
from hrl.cluts import apply_color_matrix, apply_inverse_color_matrix, invert_color_matrix
...
def test_XYZ_RGB_conversion_roundtrip_image(seed, shape):
    ...
    xyz = apply_color_matrix(rgb, M, dark)
    rgb_back = apply_inverse_color_matrix(xyz, invM, dark)
...
def test_apply_color_matrix_adds_dark_chromaticity(dark):
    M = np.eye(3)
...
def test_apply_inverse_color_matrix_subtracts_dark_chromaticity(dark):
    M = np.eye(3)
    invM = invert_color_matrix(M)
