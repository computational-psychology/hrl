...
prsr.add_argument(
    ...
    default=1.0,
    type=float,
    help="The size of the central patch, as a fraction of the total screen size. Default: 1.0",
)
...
prsr.add_argument(
    ...
    default=5,
    type=int,
    help="The number of spatial cycles of the wave. Default: 5",
)
...
prsr.add_argument(
    ...
    default=260.0,
    type=float,
    help="The temporal frequency in Hz of the applied phase shift. Default: 264.0",
)
...
prsr.add_argument(
    ...
    default=2000,
    type=int,
    help="The number of cycles to run the experiment. Default: 2000",
)
...
def main():
    ...
    wv1 = np.array(
        [[round((np.sin(x * np.pi * 2) + 1) / 2)] for x in np.linspace(0, args.scyc, phght)]
    )

    wv2 = np.array(
        [
            [round((np.sin(x * np.pi * 2 + np.pi) + 1) / 2)]
            for x in np.linspace(0, args.scyc, phght)
        ]
    )
