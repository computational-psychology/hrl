...
# Argument parser
prsr = ap.ArgumentParser(description= "Implementation of Felix test 1.  This script runs across the entire luminance range, either normally, randomly, or reversedly. This is useful for calculating the Gamma function, and Hysterisis can be measured by running several tests and comparing them. Escape can be pressed to cancel the program while it's running, and the results accumulated so far will be saved.")

prsr.add_argument('flnm',default='gamma.csv',nargs='?',help="The filename of the file where the results will be stored in the results folder. Default: gamma.csv")
...
prsr.add_argument('-t',dest='tst',default=None,type=str,help='Give a file name to test a given gamma table. Default: None')
...
prsr.add_argument('-a',dest='avg',action='store_true',help='Shall we average the samples at a particular intensity? Default: False')
