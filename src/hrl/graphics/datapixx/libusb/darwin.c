...
//zzz If I use async USB I/O there seems to be a bug.
// After dpxutil closes and reopens the USB device, I/O completion callbacks stop getting called.
// The actual USB writes are being done, but the callbacks can't tell the app that the write succeeded.
// Oh well, use sync for now.  Maybe come back to it later.
// Now when I use ASYNC it works fine.  I'll stay with the ASYNC because I think it releases the CPU better.
// NOPE. Sometimes it works, sometimes not.  Back to disabled.
// Test this using the dpxutil "reset" command, then trying another command afterwards.
// Next bulk transfer fails.
// Now it's working.  If I start with an unconfigured EZ, then download FW, then program FPGA, the FPGA reprogram works with async, but hangs with sync.
// Async is also better for using less CPU.
// Now that I'm closing/opening with every command, ASYNC gives me an "unconfigured FPGA" error.
// Under OS X 10.6.1, setting to 1 caused VPixx to hang on startup.
#define USE_ASYNC_USB	0
...
int usb_os_open(usb_dev_handle *dev)
{
  ...
  device = (struct darwin_dev_handle*)calloc(1, sizeof(struct darwin_dev_handle));
  if (!device)
    USB_ERROR(-ENOMEM);
...
int usb_os_close(usb_dev_handle *dev)
{
  ...
  if ((device = (struct darwin_dev_handle*)dev->impl_info) == NULL)
    USB_ERROR(-ENOENT);
...
static int get_endpoints (struct darwin_dev_handle *device)
{
  ...
  device->endpoint_addrs = (unsigned char*)calloc (sizeof (unsigned char), numep);
...
static int claim_interface (usb_dev_handle *dev, int interface)
{
  ...
  device = (struct darwin_dev_handle*)dev->impl_info;
...
{
  struct darwin_dev_handle *device = (struct darwin_dev_handle*)dev->impl_info;
...
//zzz Code to log write frames
#if 0
FILE* fp;
fp = fopen("/usb_write_log", "at");
int i;
fprintf(fp, "\n\nep=%d, size=%d\n", ep, size);
for (i = 0; i < size; i+=2)
fprintf(fp, "i=%d, d=0x%04X\n", i/2, *(unsigned short*)(bytes+i));
fclose(fp);
#endif
  ...
  if ((pipeRef = ep_to_pipeRef(device, ep)) < 0)
	USB_ERROR_STR ( -EINVAL, "usb_bulk_transfer: Invalid pipe reference" );
	if (ep > 1 && (size & 1)) {
		fprintf(stderr, "ERROR: Odd bytecount (%d) written to EP%d\n", size, ep);
		return -1;
	}

//zzz	result = ((*(device->interface))->WritePipe)(device->interface, pipeRef, bytes, size);
	result = ((*(device->interface))->WritePipeTO)(device->interface, pipeRef, bytes, size, timeout, timeout);	// Give it 2x timeout

  if (result != kIOReturnSuccess) {
fprintf(stderr, "ERROR: WritePipeTO() returned %d\n", result);	//zzzI've been getting some transfer errors?
    USB_ERROR_STR (result, "SYNC usb_bulk_write() error");
	return -1;
	}
  return size;
}
...
int usb_bulk_read(usb_dev_handle *dev, int ep, char *bytes, int size, int timeout)
{
  ...
  int pipeRef;
  int result;
  UInt32 packetLength = size;

  if (dev == NULL || dev->impl_info == NULL)
    return -EINVAL;

	if (ep > 1 && (size & 1)) {
		fprintf(stderr, "ERROR: Odd bytecount (%d) read from EP%d\n", size, ep);
		return -1;
	}
  ...
  if ((pipeRef = ep_to_pipeRef(device, ep)) < 0)
	USB_ERROR_STR ( -EINVAL, "usb_bulk_transfer: Invalid pipe reference" );
//zzz	result = ((*(device->interface))->ReadPipe)(device->interface, pipeRef, bytes, &packetLength);
	result = ((*(device->interface))->ReadPipeTO)(device->interface, pipeRef, bytes, &packetLength, timeout, timeout);	// Give it 2x timeout
  if (result != kIOReturnSuccess) {
fprintf(stderr, "ERROR: ReadPipeTO() returned %d\n", result);	//zzzI've been getting some transfer errors?
//zzz    USB_ERROR_STR (result, "SYNC usb_bulk_write() error");
    USB_ERROR_STR (result, "SYNC usb_bulk_read() error");
	return -1;
	}
  return packetLength;
}
...
int usb_control_msg(usb_dev_handle *dev, int requesttype, int request,
		    int value, int indexx, char *bytes, int size, int timeout)
{
  struct darwin_dev_handle *device = (struct darwin_dev_handle*)dev->impl_info;
  ...
  if (usb_debug >= 3)
    fprintf(stderr, "usb_control_msg: %d %d %d %d %p %d %d\n",
            requesttype, request, value, indexx, bytes, size, timeout);
