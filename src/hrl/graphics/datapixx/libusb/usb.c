...
usb_dev_handle *usb_open(struct usb_device *dev)
{
  ...
  udev = (usb_dev_handle *)malloc(sizeof(*udev));
  if (!udev)
    return NULL;
...
int usb_get_string(usb_dev_handle *dev, int indexx, int langid, char *buf,
	size_t buflen)
{
  /*
  ...
  return usb_control_msg(dev, USB_ENDPOINT_IN, USB_REQ_GET_DESCRIPTOR,
			(USB_DT_STRING << 8) + indexx, langid, buf, buflen, 1000);
}

int usb_get_string_simple(usb_dev_handle *dev, int indexx, char *buf, size_t buflen)
{
  ...
  ret = usb_get_string(dev, indexx, langid, tbuf, sizeof(tbuf));
  if (ret < 0)
    return ret;
