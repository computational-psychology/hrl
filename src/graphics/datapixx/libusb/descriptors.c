	...
	unsigned char type, unsigned char indexx, void *buf, int size)
{
  ...
  return usb_control_msg(udev, ep | USB_ENDPOINT_IN, USB_REQ_GET_DESCRIPTOR,
                        (type << 8) + indexx, 0, (char*)buf, size, 1000);
}
	...
	unsigned char *buffer, int size)
{
  ...
  while (size >= INTERFACE_DESC_LENGTH) {
    interface->altsetting = (struct usb_interface_descriptor*)realloc(interface->altsetting, sizeof(struct usb_interface_descriptor) * (interface->num_altsetting + 1));
