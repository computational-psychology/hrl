...
RGB to XYZ matrix. It is applied per channel by `gamma_correct_RGB`, which lives
in `hrl/cluts.py`. The module docstrings of `hrl/luts.py` and `hrl/cluts.py` are
the authority on the two formats.
```

```{important}
`hrl.luts.create_lut` and `hrl.cluts.create_clut` build a LUT or a CLUT from a
...
| `-p`, `--photometer` | `minolta` (default) for the Minolta LS-100, `optical` for the OptiCAL, or `i1pro` for the X-Rite i1Pro. |
...
constructor. The Minolta and the OptiCAL are read over a serial connection on
`/dev/ttyUSB0`. The i1Pro is accessed through VPixx's `pypixxlib`, and runs its
own interactive calibration as soon as the measurement starts.
...
and read off the first. `hrl.luts.lum_to_grey` does this; see
[deploying your code in the lab](../getting-started/deploying-in-lab) for how
to use it.
