...
[structure of an experiment](../useful-examples/experiment-structure) explains the module layout that every
template here reuses, and [managing data](../useful-examples/managing-data)
explains the design and results files they read and write. The pages below rely
on both without further comment.
...
Every template is built from the same six modules, `run_experiment.py`,
`design.py`, `experiment_logic.py`, `data_management.py`, `stimuli.py` and
`text_displays.py`. What each one is responsible for, and how they call each
other, is explained on the
[structure of an experiment](../useful-examples/experiment-structure) page.

The modules are deliberately duplicated in each folder rather than factored into
a shared package: a template is meant to be copied whole and then edited, and a
shared package would mean that editing one experiment silently changes another.

When adapting a template, you will mostly be editing `design.py`,
`experiment_logic.py` and `stimuli.py`. `data_management.py` is usually left
alone.
