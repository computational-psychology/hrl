...
that need only `HRL`, `numpy` and `Pillow`, plus `full_experiment/`, a complete
experiment in the same style that also needs `pandas`. Those are the scripts narrated in
...
- `luminance_to_intensity.py` converts a desired luminance in candela per
  square meter into the input intensity to put in a stimulus array, by
  interpolating a measured `lut.csv`. The library's own, linear version is
  `hrl.luts.lum_to_grey`; the "Deploying your code in the lab" page explains
  when the two differ.
- `example_photometer_readout.py` steps through ten intensities in random
  order, shows a uniform patch at each, and prints the luminance read by an
  X-Rite i1Pro.
